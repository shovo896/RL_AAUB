...
\begin{table}[t]
\caption{Controller Actions and Boundary Behavior}
\label{tab:actions}
\centering
\footnotesize
\begin{tabular}{@{}lll@{}}
\toprule
Action & Changed coordinate & Boundary result \\
\midrule
increase/decrease hidden & $H$ & clip to 32 or 128 \\
increase/decrease layers & $L$ & clip to 1 or 2 \\
increase/decrease dropout & $D$ & clip to 0 or 0.4 \\
keep & none & self-transition \\
\bottomrule
\end{tabular}
\end{table}
...
\footnotesize
\textbf{Input:} state set $\mathcal{S}$, actions $\mathcal{U}$, loaders,
$s_0=(64,1,0)$, $\alpha=0.4$, $\gamma=0.8$, $\epsilon=0.5$.\\[1mm]
\textbf{Initialize:} $Q(s,a)=0$ for every $(s,a)$, previous checkpoint
$\theta^-\leftarrow\varnothing$, and log $\mathcal{B}\leftarrow\varnothing$.\\[1mm]
\textbf{For} $k=1,\ldots,12$:\\
\hspace*{2mm}1) choose $a_k$ uniformly with probability $\epsilon$; otherwise choose $\arg\max_a Q(s_k,a)$;\\
\hspace*{2mm}2) obtain $s_{k+1}=T(s_k,a_k)$ and build its LSTM;\\
\hspace*{2mm}3) load $\theta^-$ if all parameter shapes match;\\
\hspace*{2mm}4) train three epochs and calculate MSE, RMSE, MAE, and $R^2$;\\
\hspace*{2mm}5) set $r_k=-\mathrm{MSE}_k$, apply \eqref{eq:qlearning}, save the checkpoint and append the metrics to $\mathcal{B}$.\\[1mm]
\textbf{Return:} $\arg\min_{b\in\mathcal{B}}\mathrm{MSE}(b)$.
\end{minipage}
\caption{Warm-start search procedure implemented by the notebook.}
\label{fig:algorithm}
\end{figure}
